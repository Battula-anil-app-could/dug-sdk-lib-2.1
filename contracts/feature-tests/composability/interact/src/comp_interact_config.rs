...
use dharitri_sc_snippets::{
    dharitri_sc::types::{EgldOrDctTokenIdentifier, TokenIdentifier},
...
impl Config {
    ...
    pub fn token_id(&self) -> EgldOrDctTokenIdentifier<StaticApi> {
        match self.token_id.as_str() {
            "MOA" => EgldOrDctTokenIdentifier::moa(),
            _ => EgldOrDctTokenIdentifier::dct(TokenIdentifier::from(self.token_id.as_str())),
        }
    }
