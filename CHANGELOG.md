...
- Dropped crate: `dharitr-wasm-output`. There is no equivalent crate, its job was passed to the individual `wasm` crates.
- `ManagedVec` supports sorting and deduplication.
- `migrateUserName` builtin function mock.

## [dharitr-wasm 0.38.0, dharitr-codec 0.16.0, mandos 0.18.0] - 2022-12-15
...
## [dharitr-wasm 0.37.0, dharitr-codec 0.15.0] - 2022-12-09
...
- `dharitr-codec` refactor: removed `TopEncodeNoErr`, `NestedEncodeNoErr` and `TypeInfo`
...
## [dharitr-wasm 0.35.0, dharitr-codec 0.13.0, mandos 0.17.0] - 2022-09-20
...
- Major refactor of the mandos-rs infrastructure.
	- High-level Mandos objects moved to dharitr-wasm-debug;
	- The `mandos` crate no longer depends on `dharitr-wasm-debug` (as originally intended and implemented);
	- Typed mandos contract call objects, for better call syntax.
	- More syntactic sugar for writing mandos calls.
- The first version of dharitr-interact-snippets, which can be used to write short blockchain interactor programs.
...
## [dharitr-wasm 0.33.1, mandos 0.15.1] - 2022-06-24
- CodecSelf for BigInt

## [dharitr-wasm 0.33.0, mandos 0.15.0] - 2022-06-20
...
## [dharitr-wasm 0.32.0, mandos 0.14.0] - 2022-06-03
...
## [dharitr-wasm 0.31.1, mandos 0.13.1] - 2022-05-04
- Bugfix - formatter single char issue.

## [dharitr-wasm 0.31.0, dharitr-codec 0.11.0, mandos 0.13.0] - 2022-05-02
...
## [dharitr-wasm 0.30.0, dharitr-codec 0.10.0] - 2022-03-17
- Feature flags in `dharitr-wasm`:
...
## [dharitr-wasm 0.28.0, dharitr-codec 0.9.0, mandos 0.12.0] - 2022-02-22
- Major dharitr-codec refactor:
...
## [dharitr-wasm 0.27.4, dharitr-codec 0.8.5] - 2022-02-02
- Backwards compatibility fix.

## [dharitr-wasm 0.27.3] - 2022-01-31
- Backwards compatibility fix.
...
## [dharitr-wasm 0.26.0] - 2022-01-19
...
## [dharitr-wasm 0.22.3] - 2021-11-10
- Memory allocation optimisations.

## [dharitr-wasm 0.22.2] - 2021-11-06
...
## [dharitr-wasm 0.21.2] - 2021-10-26
- Bugfix regarding contract upgrade args in `dharitr-wasm-debug`

## [dharitr-wasm 0.21.1, dharitr-codec 0.8.1, mandos 0.11.1] - 2021-10-26
- Relative path improvements and fixes in `dharitr-wasm-debug`:
...
## [dharitr-wasm 0.21.0, dharitr-codec 0.8.0, mandos 0.11.0] - 2021-10-22
- Mandos support for NFT syntax. Many more small improvements and some major refactoring.
- Major refactoring of the `dharitr-wasm-debug` crate, which enables the debugger and the coverage tool. Many features added:
...
## [dharitr-wasm 0.20.1] - 2021-10-05
...
## [dharitr-wasm 0.20.0, dharitr-codec 0.7.0, mandos 0.10.0] - 2021-10-02
...
## [dharitr-wasm 0.19.1] - 2021-09-17
- Legacy Send API implementation fix

## [dharitr-wasm 0.19.0, dharitr-codec 0.6.0, mandos 0.9.0] - 2021-09-10
- Managed types used extensively. Because of this, the recommended Arwen minimum version is `v1.4.10`.
	- Redesigned parts of the dharitr-codec, so as to allow custom type specializations. These specializations allow serializers and types to bypass the limitations of the codec traits to provide optimized implementations. Managed type serialization relies on this.
...
## [dharitr-wasm 0.18.2] - 2021-08-20
...
## [dharitr-wasm 0.18.1] - 2021-08-05
...
## [dharitr-wasm 0.18.0, mandos 0.8.0] - 2021-07-28
...
- `dharitr-wasm-derive` now an optional dependency of `dharitr-wasm`. Use `#[dharitr_wasm::contract]` instead of `#[dharitr_wasm_derive::contract]` now. Same for proxies and modules.

## [dharitr-wasm 0.17.4] - 2021-06-30
- conversions from big ints to small int: `BigUint::to_u64`, `BigInt::to_i64`

## [dharitr-wasm 0.17.3] - 2021-06-11
- `SingleValueMapper` `set_if_empty` method

## [dharitr-wasm 0.17.2] - 2021-06-04
- callbacks can now declared in modules only (manual forwarding from the main contract no longer required)

## [dharitr-wasm 0.17.1] - 2021-06-04
- `legacy-nft-transfer` feature for interacting with older versions of Arwen

## [dharitr-wasm 0.17.0] - 2021-05-28
...
## [dharitr-wasm 0.16.2, mandos 0.7.2] - 2021-05-20
...
## [dharitr-wasm 0.16.1, mandos 0.7.1] - 2021-05-18
- Improvements in mandos-rs: username, contract owner, nested async calls

## [dharitr-wasm 0.16.0, mandos 0.7.0, dharitr-codec 0.5.3] - 2021-05-14
...
- We no longer have a separate syntax/parser/code generation for call proxies. They are just contracts with no implementations and annotated with `#[dharitr_wasm_derive::proxy]` instead of `#[dharitr_wasm_derive::contract]`.
...
- Minor refactoring in the dharitr-codec 

## [dharitr-wasm 0.14.2] - 2021-03-29
- Fixed contract call/callback logs in mandos-rs

## [dharitr-wasm 0.14.1] - 2021-03-25
- Unified variadic arguments with respective variadic results

## [dharitr-wasm 0.14.0, mandos 0.6.0, dharitr-codec 0.5.1] - 2021-03-22
- DCT functionality:
...
- `dharitr-codec-derive` dix - `TopDecodeOrDefault` works with generics
- Upgraded to Rust2021.

## [dharitr-wasm 0.13.0] - 2021-03-04
### Main feature
- Events revamped:
	...
	- topics and data are serialized using the dharitr-codec instead of the old macro-based solution
	- old events are still allowed for now via the `#[legacy_event("0x...")]` syntax; might be removed in the future
### Refactoring 
- Major refactoring of dharitr-wasm-derive. This doesn't change much of the functionality, though.
### Minor features
- SingleValueMapper redesigned for easier use. It no longer keeps the storage value cached.

## [dharitr-wasm 0.12.0] - 2021-02-25
...
## [dharitr-wasm 0.11.0, dharitr-codec 0.5.0, mandos 0.5.0] - 2021-02-05
### Refactor
...
- Call Value API refactor and `#[payable]` updates:
	...
	- Refactoring:
    	...
    	- an extensive refactoring of dharitr-wasm-derive not yet performed, will come soon
### Minor features
...
## [dharitr-wasm 0.10.5] - 2021-01-27
- Temporary fix: callbacks allow error message argument to be missing

## [dharitr-wasm 0.10.4, mandos 0.4.2] - 2021-01-13
- Codec derive with defaults
- Storage mapper infrastructure

## [dharitr-wasm 0.10.3] - 2020-12-29
- ABI generation of endpoint output names

## [dharitr-wasm 0.10.2, dharitr-codec 0.4.2] - 2020-12-28
- Codec type hygene

## [dharitr-wasm 0.10.1, dharitr-codec 0.4.1, mandos 0.4.1] - 2020-12-23
- Minor fixes, support for strings

## [dharitr-wasm 0.10.0, dharitr-codec 0.4.0] - 2020-12-21
- Codec derive
- ABI generation framework
- New example contracts

## [dharitr-wasm 0.9.8, dharitr-codec 0.3.2, mandos 0.3.1] - 2020-11-23
- SC deploy API

## [dharitr-wasm 0.9.7, dharitr-codec 0.3.1, mandos 0.3.0] - 2020-11-11
...
## [dharitr-wasm 0.9.6] - 2020-11-09
- H256 & BoxedBytes fixes

## [dharitr-wasm 0.9.5] - 2020-11-09
- H256 is_zero, minor fixes

## [dharitr-wasm 0.9.4] - 2020-11-09
- BoxedBytes
	- optimized allocation, used in hooks
	- used for error messages

## [dharitr-wasm 0.9.3] - 2020-11-08
- Optimized Address/H256 hooks

## [dharitr-wasm 0.9.2] - 2020-11-06
- Allow slices as arguments 
- `storage_is_empty` annotation

## [dharitr-wasm 0.9.1] - 2020-11-05
- BigUint serialization bugfix

## [dharitr-wasm 0.9.0, dharitr-codec 0.3.0, mandos 0.2.0] - 2020-11-04
...
- Completely ditched the approach from dharitr-wasm 0.8.0.

## [dharitr-wasm 0.8.0, dharitr-codec 0.2.0] - 2020-11-02
...
- 4 crates:
	- dharitr-wasm
	- dharitr-wasm-derive for macros
	- dharitr-wasm-node for wasm
	- dharitr-wasm-debug for debugging and early tests
